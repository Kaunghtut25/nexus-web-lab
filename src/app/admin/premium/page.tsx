...
export default function PremiumFeaturesPage() {
  ...
  useEffect(() => {
    fetch('/api/premium-features').then(r => r.json()).then(d => setFeatures(d.premiumFeatures || []));
  }, []);

  async function save() {
    await fetch('/api/premium-features', { method: 'POST', headers: apiHeaders(), body: JSON.stringify(edit) });
    const d = await fetch('/api/premium-features').then(r => r.json());
    setFeatures(d.premiumFeatures || []);
    setEdit(null);
  }

  async function remove(id: string) {
    await fetch('/api/premium-features', { method: 'DELETE', headers: apiHeaders(), body: JSON.stringify({ id }) });
    setFeatures(s => s.filter(x => x.id !== id));
  }

  return (
    <AuthGuard>
      <AdminLayout title="Premium Features">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-navy">Premium Package Features ({features.length})</h2>
        ...
        {edit && (
          <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-4">
              ...
              <div><label className="text-xs text-slate-500 block mb-1">Icon (emoji)</label><input value={edit.icon || ''} onChange={e => setEdit({ ...edit, icon: e.target.value })} placeholder="🛒 📊 💳 🤖 🎧 ♾️" className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:border-blue" /></div>
