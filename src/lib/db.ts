...
// Learning system: knowledge base + experience pool + visitor feedback
await client.execute(`CREATE TABLE IF NOT EXISTS kb_entries (id TEXT PRIMARY KEY, question TEXT NOT NULL, answer TEXT NOT NULL, lang TEXT DEFAULT 'en', source TEXT DEFAULT 'manual', usage_count INTEGER DEFAULT 0, created_at TEXT DEFAULT (datetime('now')))`);
await client.execute(`CREATE TABLE IF NOT EXISTS chat_learning (id TEXT PRIMARY KEY, visitor_id TEXT, question TEXT NOT NULL, bot_reply TEXT DEFAULT '', status TEXT DEFAULT 'unanswered', source TEXT DEFAULT 'fallback', created_at TEXT DEFAULT (datetime('now')))`);
await client.execute(`CREATE TABLE IF NOT EXISTS chat_feedback (id TEXT PRIMARY KEY, visitor_id TEXT, question TEXT DEFAULT '', reply TEXT DEFAULT '', rating INTEGER NOT NULL, created_at TEXT DEFAULT (datetime('now')))`);
