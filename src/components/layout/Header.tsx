...
export default function Header() {
  ...
  return (
    <header className={`sticky top-0 transition-all duration-300 ${
      ...
      solid
        ? 'bg-[#2563EB]/85 backdrop-blur-md shadow-lg shadow-black/20 border-b border-white/10'
      ...
      {open && (
        <div className="lg:hidden absolute left-0 right-0 top-full bg-[#2563EB]/95 border-t border-white/10 px-4 pb-5 shadow-2xl shadow-black/30 max-h-[85vh] overflow-y-auto">
          {nav.map((n) => {
            const active = isActive(n.href);
            const Icon = n.icon;
            return (
